...
from .postgres import get_postgres_engine
from .sqlite import get_sqlite_engine
...
if DB_TYPE == "sqlite":
    DATABASE_URL = os.getenv("SQLITE_URL", "sqlite+aiosqlite:///./etzan.db")
    engine, async_session_maker = get_sqlite_engine(DATABASE_URL)
else:
    DATABASE_URL = os.getenv("DATABASE_URL")
    engine, async_session_maker = get_postgres_engine(DATABASE_URL)
...
async def init_db():
    ...
    print(f"[DB] Database tables created/verified successfully using {DB_TYPE}.")
